...
export class ElectronAgentHostStarter extends Disposable implements IAgentHostStarter {
	...
	async start(): Promise<IAgentHostConnection> {
		...
		// VS Agent: point the Claude Agent SDK at the copy bundled in the app
		// (`<appRoot>/node_modules/@anthropic-ai/claude-agent-sdk`). This makes the
		// built app's SDK-availability gate pass and the SDK load without a Copilot
		// subscription or a CDN download. A real dev override on the parent env still wins.
		if (process.env[AgentHostClaudeSdkRootEnvVar] === undefined) {
			sdkEnv[AgentHostClaudeSdkRootEnvVar] = this._environmentMainService.appRoot;
		}
